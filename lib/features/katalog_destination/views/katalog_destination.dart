...
class KatalogProdukPage extends StatefulWidget {
  ...
  const KatalogProdukPage({Key? key, this.categoryName, this.categoryColor})
      : super(key: key);
...
class _KatalogProdukPageState extends State<KatalogProdukPage> {
  ...
  void initState() {
    super.initState();
    debugPrint(
        '🎨 KatalogProdukPage initialized with categoryColor: ${widget.categoryColor}');
    _initializeData();
  }
  ...
  Future<void> _initializeData() async {
    try {
      ...
      if (widget.categoryName != null) {
        ...
        debugPrint(
            '🎯 Selected category: ${_selectedCategory!.categoryName} (ID: ${_selectedCategory!.id})');
      } else {
        _selectedCategory = categories.first;
        debugPrint(
            '🎯 Using first category: ${_selectedCategory!.categoryName}');
      }

      // If categoryColor wasn't passed through widget, try to get it from selected category
      String? effectiveCategoryColor = widget.categoryColor;
      if (effectiveCategoryColor == null && _selectedCategory != null) {
        // Assuming CategoryModel has a color property
        // effectiveCategoryColor = _selectedCategory!.color;
        debugPrint('🎨 No category color passed, using category default');
      }
      ...
      if (destinations.isNotEmpty) {
        for (int i = 0;
            i < (destinations.length > 3 ? 3 : destinations.length);
            i++) {
          debugPrint(
              '  ${i + 1}. ${destinations[i].title} (${destinations[i].subcategory})');
        }
      }
      ...
      debugPrint(
          '✅ Successfully loaded ${destinations.length} destinations for category: ${_selectedCategory!.categoryName}');
      debugPrint('📂 Available subcategories: $_subcategories');
      debugPrint(
          '🔍 Initial filtered destinations: ${_filteredDestinations.length}');
      debugPrint('🎨 Final category color being used: ${widget.categoryColor}');
    } catch (e, stackTrace) {
  ...
  void _filterDestinations(String query) {
    ...
    _debounceTimer = Timer(const Duration(milliseconds: 300), () {
      ...
      debugPrint(
          '📊 Total destinations to filter: ${_destinationsList.length}');
      ...
      setState(() {
        ...
        if (_searchQuery.isEmpty) {
          ...
          debugPrint(
              '🔄 Query empty, showing all ${_filteredDestinations.length} destinations');
        } else {
          _filteredDestinations = _destinationsList.where((destination) {
            ...
            final matches =
                titleMatches || addressMatches || subcategoryMatches;

            // Debug logging for first few items
            if (_destinationsList.indexOf(destination) < 3) {
              debugPrint(
                  '  🔎 "${destination.title}": title=$titleMatches, address=$addressMatches, subcat=$subcategoryMatches → $matches');
            }
          ...
          debugPrint(
              '✅ Filtered to ${_filteredDestinations.length} destinations for query "$query"');
          ...
          if (_filteredDestinations.isNotEmpty) {
            ...
            for (int i = 0;
                i <
                    (_filteredDestinations.length > 3
                        ? 3
                          ...
                          child: Column(
                            ...
                            children: [
                              ...
                              ProductGridSection(
                                ...
                                userId:
                                    GoogleAuthService.getAuthenticatedUserId(),
                                selectedCategory: _selectedCategory!.id!,
                                categoryColor: widget
                                    .categoryColor, // Make sure this is passed
                              ),
                            ],
                          ),
                        ),
